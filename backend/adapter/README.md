...
Spring Boot service for Push webhook ingestion, pre-filtering, local buffering, and forwarding to `fm-module`.

**Stack:** Java 25, Spring Boot 3.4, PostgreSQL 15+, Liquibase  
**Port:** 8081  
...
| `FM_MODULE_BASE_URL` | `http://localhost:8080` | fm-module base URL for ingest |
...
1. **Webhook** — validates API key against `source_config_snapshots`, applies `filter_rules`, forwards to `POST {FM_MODULE_BASE_URL}/api/v1/ingest?sourceKey={key}`.
...
3. **Buffer** — when fm-module is unreachable, payload is stored in `buffered_messages`; background worker retries delivery.
